/**
 * packages/extension/oauthguard/tests/state_parameter_abuse.test.js
 *
 * Vitest unit tests for the OAuth state parameter email encoding detector.
 * No network calls, no Chrome extension APIs required.
 *
 * Run: npx vitest run  (from packages/extension/)
 */

import { describe, it, expect } from 'vitest';
import { detectStateParameterAbuse } from '../detectors/state_parameter_abuse.js';

// ---------------------------------------------------------------------------
// Test fixtures — pre-computed base64 values for readability
// ---------------------------------------------------------------------------

// btoa('victim@corp.com') === 'dmljdGltQGNvcnAuY29t'
const VICTIM_EMAIL_B64 = 'dmljdGltQGNvcnAuY29t';

// btoa('test@example.com') === 'dGVzdEBleGFtcGxlLmNvbQ=='
// URL-safe: remove padding, replace + → -, / → _
const TEST_EMAIL_B64_URLSAFE = btoa('test@example.com')
  .replace(/\+/g, '-')
  .replace(/\//g, '_')
  .replace(/=/g, '');

// Hex encoding of 'admin@target.org'
const ADMIN_EMAIL_HEX = Array.from('admin@target.org')
  .map(c => c.charCodeAt(0).toString(16).padStart(2, '0'))
  .join('');

// ---------------------------------------------------------------------------
// Happy path: detection must fire
// ---------------------------------------------------------------------------
...
describe('detectStateParameterAbuse — detection cases', () => {
  ...
  it('stateValue in result is truncated to 64 chars max', () => {
    // Use a long encoded email to test PII truncation
    ...
    // Whether detected depends on email regex — the key assertion is no crash
...
describe('detectStateParameterAbuse — clean cases', () => {
  ...
  it('does not fire when state param is a standard CSRF token (random hex)', () => {
    const url =
      'https://login.microsoftonline.com/common/oauth2/authorize' +
  ...
  it('does not fire when state param decodes to a non-email string', () => {
    // base64 of 'hello world' — decodable but not an email
    const url =
      'https://login.microsoftonline.com/common/oauth2/authorize' +
  ...
  it('does not fire on a clean Microsoft sign-in URL with legitimate state', () => {
    // Real MSAL state token — base64 JSON, decodes to JSON not an email
  ...
  it('does not fire when state is binary (non-printable base64 payload)', () => {
    // base64 of raw bytes that don't decode to printable ASCII
...
// ---------------------------------------------------------------------------
// Error resilience: must never throw
// ---------------------------------------------------------------------------
...
describe('detectStateParameterAbuse — error resilience', () => {
  ...
  it('always returns a non-empty signals array regardless of input', () => {
    const inputs = [
      null,
      '',
      'not-a-url',
  ...
  it('always returns empty string fields (not null/undefined) when not detected', () => {
