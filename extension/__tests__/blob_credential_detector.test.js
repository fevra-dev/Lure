/**
 * extension/__tests__/blob_credential_detector.test.js
 *
 * Vitest unit tests for the blob: URL credential page detector.
 * Migrated from wave2/blob_credential_detector.test.js with updated import paths.
 *
 * Run: npx vitest run
 */
...
describe('countPasswordFields', () => {
  ...
  it('returns 2 for a confirm-password pattern', () => {
    setBody('<form><input type="password" name="password"><input type="password" name="confirm_password"></form>');
...
describe('checkNestedSmuggling', () => {
  ...
  it('counts each script tag once', () => {
    setBody('<script>var d = atob("x"); var b = new Blob([d]); URL.createObjectURL(b);</script>');
    const result = checkNestedSmuggling();
    expect(result.patternCount).toBe(1);
  });

  it('counts multiple script tags separately', () => {
    setBody('<script>atob("PHg+")</script><script>new Blob(["x"])</script>');
    const result = checkNestedSmuggling();
    expect(result.patternCount).toBe(2);
  });

  it('does not flag external script tags', () => {
...
describe('calculateRiskScore', () => {
  ...
  it('returns 0.0 when no signals are present', () => {
    const { riskScore, signalList } = calculateRiskScore({
      passwordFieldCount: 0, matchedBrands: [],
      nestedSmugglingDetected: false, nestedPatternCount: 0,
      formExfiltrationDetected: false, externalActions: [],
    });
  ...
  it('scores 0.50 for a single password field alone', () => {
    const { riskScore } = calculateRiskScore({
      passwordFieldCount: 1, matchedBrands: [],
      nestedSmugglingDetected: false, nestedPatternCount: 0,
      formExfiltrationDetected: false, externalActions: [],
    });
  ...
  it('credential_field + brand = 0.70 (alert threshold met)', () => {
    const { riskScore, signalList } = calculateRiskScore({
      passwordFieldCount: 1, matchedBrands: ['microsoft'],
      nestedSmugglingDetected: false, nestedPatternCount: 0,
      formExfiltrationDetected: false, externalActions: [],
    });
  ...
  it('full signal set caps at 1.0', () => {
    const { riskScore } = calculateRiskScore({
      passwordFieldCount: 2, matchedBrands: ['microsoft'],
      nestedSmugglingDetected: true, nestedPatternCount: 2,
      formExfiltrationDetected: true, externalActions: ['https://attacker.com/steal'],
    });
    expect(riskScore).toBe(1.0);
  });

  it('scores 0.90 for canonical high-risk page', () => {
    const { riskScore } = calculateRiskScore({
      passwordFieldCount: 1, matchedBrands: ['microsoft'],
      nestedSmugglingDetected: true, nestedPatternCount: 1,
      formExfiltrationDetected: false, externalActions: [],
    });
  ...
  it('signals list is populated correctly', () => {
    const { signalList } = calculateRiskScore({
      passwordFieldCount: 2, matchedBrands: ['paypal', 'google'],
      nestedSmugglingDetected: true, nestedPatternCount: 1,
      formExfiltrationDetected: true, externalActions: ['https://steal.ru/c'],
    });
...
describe('disableCredentialFields', () => {
  ...
  it('disables all password fields', () => {
    setBody('<form><input type="password" id="p1"><input type="password" id="p2"></form>');
...
describe('full detection scenario', () => {
  ...
  it('high-confidence phishing page: password + Microsoft brand + nested + exfil', () => {
    setTitle('Sign in to Microsoft');
    setBody(`
    ...
    const { riskScore, signalList } = calculateRiskScore({
      passwordFieldCount: pwCount, matchedBrands,
      nestedSmugglingDetected: nested, nestedPatternCount: 1,
      formExfiltrationDetected: exfil, externalActions,
    });
